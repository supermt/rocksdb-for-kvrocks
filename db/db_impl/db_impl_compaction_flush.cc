...
Status DBImpl::CompactRange(const CompactRangeOptions& options,
  ...
  return CompactRangeInternal(options, column_family, begin_with_ts,
                              end_with_ts, "" /*trim_ts*/, compact_results);
}
...
Status DBImpl::RunManualCompaction(
  ...
  // manual compaction is finished, no matter how
  if (result_names != nullptr && !manual.result_file_names.empty()) {
    // insert the entire data
    for (auto file : manual.result_file_names) {
      result_names->push_back(file);
    }
  }
  log_buffer.FlushBufferToLog();
  assert(!manual.in_progress);
    ...
    *made_progress = true;
    ...
    if (manual_compaction != nullptr) {
      for (const auto& newf : c->edit()->GetNewFiles()) {
        ...
        manual_compaction->result_file_names.push_back(TableFileName(
